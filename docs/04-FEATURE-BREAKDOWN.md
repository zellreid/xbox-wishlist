...
**Why:** Live feedback 2026-09-25: chips should always appear in one agreed order, and owned items showed an empty gap between the "Owned" line and the chip row. Confirmed live 2026-09-25.

**T-24 - Filter panel dividers (v1.5.26268.11), then layout (v1.5.26268.12) - Done, needs a live check:** v1.5.26268.12 moves Saved filters straight after the search box (`addSavedPresets()` inserts after the search wrapper) and wraps the quick filters in a section with a "Quick filters" heading; both use `.ifc-panel-section` (divider + spacing above) and `.ifc-section-heading` (the former saved-filters heading style). Panel order: header, search, Saved filters, Quick filters, active tags, filter groups. Harness: that order on all 5 mocks and `?public`; saving a preset still works. v1.5.26268.14: range sections share tighter spacing (`.ifc-slider-container` padding 6px 10px / no margin, label margin 12px, track margin 0): heading +16 label +12 slider +16, both 110px (Price 111 with its divider), was 151; `ul.filter-groups` margin-bottom 0 (Xbox generic `ul` 1rem), so the panel foot is its 15px padding, same as the top; slider drag still filters (152 of 310 with the tag). v1.5.26268.13 drops the last group's bottom border (`.ifc-accordion-group:last-child`; only the filter panel uses these groups; Discount Range on all 5 mocks). v1.5.26268.11: the tag row's divider moved from its bottom to its top (`.ifc-tag-container` border-top + margin-top), so tags sit between it and the first group's top border instead of two lines stacking under them; hidden with no tags, leaving one line. `.ifc-saved-presets` lost its border-top and padding (the last group's bottom border already separates it). Harness: with a tag, lines at quick filters +10px, tags, first group; without, one line; nothing above Saved filters. PASS on all 5 mocks.
...
- **Per market (v1.5.26269.1):** the market is the region in the URL (`/en-ZA/` -> `ZA`, `getMarket()`); the history is saved under `ifc_xbox_wishlist_prices_<market>` so switching to en-US never mixes rand and dollar prices. History saved before this (single key, all rand) is read as ZA the first time ZA has none. The currency symbol is read from the page's own price text (`readPrice()`) instead of the fixed "R", and `parsePriceText()` reads both `R1 299,00` and `$1,299.00` (the old parser read the latter as 1.299). Not yet per market: the saved price range and saved filters (T-25), and the fixed English labels such as Owned (T-26).

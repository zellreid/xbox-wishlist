(function () {
    ...
    function saveFilterState() {
        try {
            ...
            chrome.storage.local.set({ [CONFIG.storage.key]: JSON.stringify(saveData) });
        } catch (ex) { console.error('Failed to save filter state:', ex); }
    }

    function loadFilterState() {
        chrome.storage.local.get([CONFIG.storage.key], (result) => {
        try {
            const saved = result[CONFIG.storage.key];
            if (saved) {
    ...
    async function updateSVGIcon(containerId, resourceKey, targetId) {
        try {
            const svgText = await getSVG(chrome.runtime.getURL(resourceKey));
            if (!svgText) return;
    ...
    function addFilterButton() {
        ...
        try {
            const bc = getElement(`#${CONFIG.ids.buttonContainer}`); if (!bc) return;
            bc.appendChild(createImageButton('Filter', chrome.runtime.getURL('icons/filter.svg'), 'Filter', 'svg'));
    ...
    function addSortButton() {
        ...
        try {
            const bc = getElement(`#${CONFIG.ids.buttonContainer}`); if (!bc) return;
            bc.appendChild(createImageButton('Sort', chrome.runtime.getURL('icons/sort.svg'), 'Sort', 'svg'));
    ...
    function createFilterBlock(id = null, text = '', collapsible = true) {
        ...
        loadSVGIntoContainer(chevronContainer, chrome.runtime.getURL('icons/expand.svg'), `accordion_${id}`);
        ...
        headerButton.addEventListener('click', async () => {
            ...
            const resourceKey = isExpanded ? 'icons/expand.svg' : 'icons/collapse.svg';
            await loadSVGIntoContainer(chevronContainer, chrome.runtime.getURL(resourceKey), `accordion_${id}`);
        });
    ...
    initialize().then(() => {
        onDOMReady();
        // SPA retry: items may not be rendered yet at document_idle
        let attempts = 0;
